...
async def main():
    ...
    # Register Telegram menu button first
    ...
    # return_exceptions=True — one crash never kills others
    await asyncio.gather(
        start_listener(),          # AI token discovery
        monitor_influencers(),     # KOL Twitter monitor
        start_wallet_tracker(),    # Autonomous wallet discovery + copy trading
        start_exit_manager(),      # Take profit / stop loss
        start_command_listener(),  # /pnl /status /positions /help
        daily_summary_loop(),      # Daily PNL at midnight
